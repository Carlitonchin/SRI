import json
from src.text_processing import dataset_processing
...
# Documents
# queries_ = ''
# for query in dataset_.queries_iter():
#     queries_ += '.I '+str(query.query_id)+'\n'
#     queries_ += '.W\n'
#     queries_ += str(query.text)
#     # print(query)
# with open('./queries', 'w') as queries:
#     queries.write(queries_)
rel_dic = {}
# rel_dic['docs']=[]
# rel_dic['relevancy_scale']=[]
for qrel in dataset_.qrels_iter():
    try:
        rel_dic[str(qrel.query_id)]['docs'].append(str(qrel.doc_id))
        rel_dic[str(qrel.query_id)]['relevancy_scale'].append(str(qrel.relevance))
    except:
        rel_dic[str(qrel.query_id)] = {'docs' : [str(qrel.doc_id)], 'relevancy_scale' : [str(qrel.relevance)]}
        
json_object = json.dumps(rel_dic, indent = 4)
with open('./rel_dic.json', 'w') as settings:
    settings.write(json_object)