...
def query_processing(query, based_md='en_core_web_sm'):
    texts = []
    text = query
    while len(text) > 10000:
        texts.append(text[: 10000])
    ...
    for text in texts:
        processed_text += processing_text(text, based_md)
    # processed_text = cleansing_query(processed_text)
    return processed_text

query_processing('what similarity laws must be obeyed when constructing aeroelastic models of heated high speed aircraft .', 'en_core_web_sm')
